...
public class CapsuleManSpawner : NetworkBehaviour
{
    ...
    public override void OnStartServer()
    {
        ...
        GameObject spawnedCapsule = Instantiate(capsuleManPrefab, new Vector3(0, 5, 0), Quaternion.identity);
        //if(spawnedCapsule.scene != gameObject.scene)
        //{
        //    UnityEngine.SceneManagement.SceneManager.MoveGameObjectToScene(spawnedCapsule, gameObject.scene);
        //}

        // 2. FishNet 서버에게 "이 객체를 스폰해라!" 라고 명령합니다.
        // 💡 씬 객체가 아니라 동적으로 스폰된 객체는 ObserverManager가 절대 렌더러를 끄지 않습니다!
        ServerManager.Spawn(spawnedCapsule);
